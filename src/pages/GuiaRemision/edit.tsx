...
interface GuiaRemisionEditFormProps {
  id?: string;
}

const GuiaRemisionEditForm = ({
  id: propGuiaId,
}: GuiaRemisionEditFormProps) => {
  ...
  const [puntoEmisionSelected, _] = useState<number>(0);
  ...
  const previewPDF = async (values: GuiaRemision) => {
    const doc = {
      fechaEmision:
        values.datosGenerales.fechaEmision + " " + dayjs().format("HH:mm"),
      ...
      envio: {
        ...
        transportista:
          values?.transportista?.numDoc !== "" ? values.transportista : null,
      },
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      try {
        ...
        const { fechaEmision, correlativo, serie, tipoDoc, version } =
        ...
        if (fecTraslado && fechaEmision) {
          ...
          if (
            isNaN(parseFechaEmision.getTime()) ||
            isNaN(parseFecTraslado.getTime())
          ) {
          ...
          const unDiaAtras = new Date(fechaActual);
          unDiaAtras.setDate(fechaActual.getDate() - 1);
          ...
          const unDiaAtrasSinHora = new Date(
            unDiaAtras.getFullYear(),
            unDiaAtras.getMonth(),
            unDiaAtras.getDate(),
          );
          ...
          if (fechaEmisionSinHora < unDiaAtrasSinHora) {
            getError(
              "La fecha de emisión no puede ser anterior a 1 día de la fecha actual",
            );
            return;
          }
          ...
          if (fechaEmisionSinHora > dosDiasAdelanteSinHora) {
            getError(
              "La fecha de emisión no puede ser posterior a 2 días de la fecha actual",
            );
            return;
          }
          ...
          if (parseFecTraslado < parseFechaEmision) {
            getError(
              "Datos de Envío: La fecha de traslado debe ser mayor a la fecha de Emisión de la Guía",
            );
            return;
          }
        }
        ...
        if (
          values.envio.modTraslado === "01" &&
          values.choferes.length > 0 &&
          values.envio.fecInicioTrasladoBienes
        ) {
          getError(
            "Datos de Envío: La fecha de inicio de traslado de bienes debe elegirse",
          );
          return;
        }
        ...
        if (idDespatch) {
          ...
          getError(
            "No puede actualizar la Guia porque no encuentra el ID de despacho",
          );
          return;
        }
        ...
        getSuccess(
          "La guía se guardó correctamente, se procede a enviar a SUNAT",
        );
        ...
        const errorMessage =
          error instanceof Error
            ? error.message
            : "Error inesperado al procesar la guía";
        getError(errorMessage);
    ...
    const loadGuiaData = async () => {
      ...
      try {
        ...
        if (response.data) {
          ...
          let vehiculos =
            guiaData.envio.vehiculo?.length > 0
              ? guiaData.envio.vehiculo?.find((ve) => ve.tipo === "P")
              : null;
          let secundarios =
            guiaData.envio.vehiculo?.length > 0
              ? guiaData.envio.vehiculo?.filter((ve) => ve.tipo === "S")
              : [];
          ...
          let detalles = guiaData.details || [];

          detalles = detalles.map((detalle) => ({
            ...detalle,
            id: detalle.id_producto,
            // Realiza las transformaciones necesarias aquí
          }));

          let transportista = guiaData.envio.transportista || null;

          if (transportista) {
            transportista.id = transportista.id_transportista || 0;
          }
          let partida = guiaData.envio.partida || null;
          if (partida) {
            partida.id = partida.id_puntoubicacion || 0;
          }
          let llegada = guiaData.envio.llegada || null;
          if (llegada) {
            llegada.id = llegada.id_puntoubicacion || 0;
          }
          ...
          formik.setValues({
            ...
            datosGenerales: {
              ...
              fechaEmision: dayjs((guiaData as any).fechaEmision).format(
                "YYYY-MM-DD",
              ),
            ...
            envio: {
              ...guiaData.envio,
              fecTraslado: dayjs(guiaData.envio.fecTraslado).format(
                "YYYY-MM-DD",
              ),
              fecInicioTrasladoBienes: guiaData.envio.fecInicioTrasladoBienes
                ? dayjs(guiaData.envio.fecInicioTrasladoBienes).format(
                    "YYYY-MM-DD",
                  )
                : "",
            },
          ...
          // console.log(guiaData.envio, 'guiaData.envio');

          // console.log(guiaData);

          // Configurar datos adicionales
          setAdicionalDocs(guiaData.addDocs || []);
          setDetalles(detalles || []);
          setIdDespatch(guiaData?.id || null);
          setEstadoElectronico(guiaData?.electronico?.id || null);

          // Mensaje usando campos directos del API
          getSuccess(
            `Guía ${(guiaData as any).serie}-${(guiaData as any).correlativo} cargada para edición`,
          );
        }
      } catch (error) {
        console.error("Error loading guia:", error);
        getError(error || "Error al cargar los datos de la guía");
        navigate("/admin/guias");
      } finally {
        setLoading(false);
      }
  ...
  const onHandleSelectSerie = (): void => {};
  ...
  const handleDeleteAdddoc = (item: AddDoc): void => {
    const filterAdicionalDocs = adicionalDocs.filter(
      (it) => it.emisor !== item.emisor && it.nro !== item.nro,
    );
  ...
  return (
    <LocalizationProvider dateAdapter={AdapterDayjs}>
      {loading ? (
        ...
        <Container maxWidth="md" sx={{ mt: 5 }}>
          <Typography textAlign={"center"} variant="h4" my={3}>
          ...
          <Box component={"form"} onSubmit={formik.handleSubmit}>
                    ...
                    <Grid item xs={6}>
                      ...
                      >
                          ...
                          onClick={(_e) =>
                            handleOpenModalForm(
                              <Cliente
                                ...
                                tipo={
                                  formik.values.envio.codTraslado === "02" ||
                                  formik.values.envio.codTraslado === "04"
                                    ? "default"
                                    : ""
                                }
                              />,
                        ...
                        <Box display={"flex"} flexDirection={"column"} mt={1}>
                          ...
                          >
                            {formik.values.destinatario?.numDoc}
                          </Typography>
                          <Typography
                            sx={{ fontSize: 11 }}
                            color={theme.palette.text.disabled}
                      ...
                      >
                        ...
                        <Box display={"flex"} flexDirection={"column"} mt={1}>
                          <Typography
                            ...
                            color={theme.palette.text.disabled}
                          >
                            {formik.values.tercero?.numDoc}
                          </Typography>
                          <Typography
                            sx={{ fontSize: 11 }}
                            color={theme.palette.text.disabled}
                          >
                            {formik.values.tercero?.rznSocial}
                          </Typography>
                        </Box>
                      </Paper>
                    </Grid>
                ...
                <AccordionDetails>
                  ...
                  <Grid item xs={12}>
                    <DocumentosAdicionales
                      onDelete={handleDeleteAdddoc}
                      adicionales={formik.values.addDocs}
                    />
                  </Grid>
                </AccordionDetails>
              </Accordion>
                ...
                <AccordionDetails>
                  ...
                  <Grid item xs={12}>
                    <DocumentosDetalles
                      ...
                      detalles={formik.values.details}
                    />
                  </Grid>
                </AccordionDetails>
              </Accordion>
                  ...
                  >
                      ...
                      <Paper elevation={5} sx={paperDirection}>
                        ...
                        >
                          <Typography sx={{ fontSize: 12 }}>
                            {formik.values.partida?.ruc}
                          </Typography>
                          <Typography sx={{ fontSize: 12 }}>
                            {formik.values.partida?.rznSocial}
                          </Typography>
                          <Typography
                            sx={{ fontSize: 11 }}
                            color={theme.palette.text.disabled}
                          >
                            {formik.values.partida?.direccion}
                          </Typography>
                        </Box>
                      </Paper>
                    </Grid>
                    <Grid item xs={12} sm={6} mt={{ xs: 1, sm: 0 }}>
                      <Paper elevation={5} sx={paperDirection}>
                        ...
                        <Box display={"flex"} flexDirection={"column"} mt={1}>
                          <Typography sx={{ fontSize: 12 }}>
                            {formik.values.llegada?.ruc}
                          </Typography>
                          <Typography sx={{ fontSize: 12 }}>
                            {formik.values.llegada?.rznSocial}
                          </Typography>
                          <Typography
                            sx={{ fontSize: 11 }}
                            color={theme.palette.text.disabled}
                          >
                            {formik.values.llegada?.direccion}
                          </Typography>
                        </Box>
                      </Paper>
                    </Grid>
                        ...
                        <Grid item sm={4} xs={6}>
                          <Box component={"div"}>
                            ...
                            <IconButton
                              ...
                              disabled={
                                formik.values.envio.indicadores.includes(
                                  "SUNAT_Envio_IndicadorTrasladoVehiculoM1L",
                                ) && formik.values.envio.modTraslado === "02"
                              }
                              sx={BoxShadoWButton}
                              onClick={(_e) =>
                                handleOpenModalForm(
                            ...
                            {formik.values.choferes.map((cho) => (
                              <Typography
                                key={cho.id}
                                sx={{
                                  fontSize: cho.tipo === "Principal" ? 11 : 10,
                                }}
                                color={
                                  cho.tipo === "Principal"
                                    ? "warning.dark"
                              ...
                              >
                                {cho.tipo.substring(0, 3).toUpperCase()}:{" "}
                                {cho.nombres} {cho.apellidos}
                              </Typography>
                            ))}
                          </Box>
                        </Grid>
                            ...
                            {formik.values?.transportista?.numDoc !== "" && (
                              ...
                              >
                                ...
                                <Typography
                                  ...
                                  color={theme.palette.text.secondary}
                                >
                                  {formik.values.transportista?.rznSocial}
                                </Typography>
                                <Typography
                                  sx={{ fontSize: 11 }}
                                  color={theme.palette.text.disabled}
                                >
                                  {formik.values.transportista?.nroMtc}
                                </Typography>
                              </Box>
                            )}
                          ...
                          <Box component={"div"}>
                                  ...
                                  onClick={(_e) =>
                                    handleOpenModalForm(
                                      <VehiculosSecundarios
                                        ...
                                        vehiculos={
                                          formik.values.vehiculo?.secundarios
                                        }
                                      />,
                            ...
                            {formik.values.vehiculo?.placa !== "" && (
                              <Typography
                                ...
                                color="warning.dark"
                              >
                                Principal: {formik.values.vehiculo?.placa}
                              </Typography>
                            )}
            ...
            <Button
              ...
              variant="contained"
              sx={{ my: 1, color: "white", fontWeight: "bold" }}
              fullWidth
            >
              Vista previa de PDF
            </Button>
...
export default GuiaRemisionEditForm;
