...
const DatosDireccion = ({
  ...
  const filterFav = async (tipo: string) => {
    try {
      ...
      const { data } = await PuntoUbicacionService.get(`${tipo}=1`);
  ...
  const handleSetFavorite = (item: ChipInterface): void => {
    ...
    formik.setFieldValue('id', punto?.id || 0)
    formik.setFieldValue('rznSocial', punto?.rznSocial || '')
    formik.setFieldValue('ubigeo', punto?.ubigeo || '')
    formik.setFieldValue('direccion', punto?.direccion || '')
    formik.setFieldValue('codLocal', punto?.codLocal || '')
    formik.setFieldValue('ruc', punto?.ruc || '')
  }
        ...
        <Box display={'flex'} flexDirection={{ md: 'row' }} alignItems={'center'} gap={1}>
          <TextField
            ...
            InputProps={{
              endAdornment: (
                <AnimatedInputAdornment position="end">
                  <LightTooltip arrow title="No uses R.U.C. si vas a enviar/recoger desde un cliente que no tiene RUC, dejalo en blanco. SUNAT hará la validación y va a notificar un ERROR">
                    <InfoIcon color="action" />
                  </LightTooltip>
                </AnimatedInputAdornment>
              ),
